...
import numpy as np
np.set_printoptions(formatter={'int': hex, 'uint8': hex})
from AES import AES
...
if __name__ == "__main__":
    ...
    message = "David Candela Rubio i Guillermo Raya Garcia"
    byte_message = np.array(list(map(ord, (message + 15 * ' ')[:(len(message) + 15) & 0xF0])))
    byte_cipher = aes.encode(message)
    
    print(f"Missatge (text): \"{message}\"")
    print(f"Missatge (hexadecimal): \"{byte_message}\"")
    print(f"Xifrat (hexadecimal): \"{byte_cipher}\"")